...
class MultiObjectiveSolver:
    ...
    def _create_decision_variables(self):
        ...
        self.W = plp.LpVariable.dicts("W", [(i, self.shifts[j].shift_date) for i in self.I for j in self.J if
                                            (i, j) in self.valid_keys], cat=plp.LpBinary)
    ...
    def _create_constraints(self):
        ...
        for i in self.I:
            self.model += self.Y[i] * DM <= (HMM - self.nurses[i].accumulated_hours) + self.V[
                i], f"overtime_{self.nurses[i].nurse_id}"
        ...
        for i in self.I:
            for j1 in self.J:
                if (i, j1) in self.valid_keys and datetime.strptime(self.shifts[j1].shift_date,
                                                                    '%Y-%m-%d').weekday() == 5:
                    self.model += self.X[i, j1] + plp.lpSum([self.X[i, j2] for j2 in self.J if
                                                             (i, j2) in self.valid_keys and datetime.strptime(
                                                                 self.shifts[j2].shift_date, '%Y-%m-%d').weekday() == 6
                                                             and datetime.strptime(self.shifts[j2].shift_date,
                                                                                   '%Y-%m-%d') == datetime.strptime(
                                                                 self.shifts[j1].shift_date, '%Y-%m-%d') + timedelta(
                                                                 days=1)
        ...
        for i in self.I:
            ...
            i, j) in self.valid_keys]), f"weekend_off_{self.nurses[i].nurse_id}"
